...
class Installer:
  ...
  def __install_server_software_in_container(self, test_name, test_dir):
    # import docker here
    c = setup_util.get_client()
    ...
    command="toolset/run-tests.py --install server --test %s --install-error-action abort --install-only --docker-client" % test_name
    testvolume="/root/FrameworkBenchmarks/%s" % test_name
  ...
  def __install_server_software(self):
    ...
    for test_install_file in install_files:
      ...
      if self.docker: 
        self.__install_server_software_in_container(test_name, test_dir)
        continue
